...
def graph_enrolledUsers_api(request):
    """
    API endpoint to retrieve enrollment records within a specified date range.

    Used for generating time-series graphs of student enrollments.

    Args:
        request (HttpRequest): Contains 'start_date' and 'end_date' (YYYY-MM-DD).

    Returns:
        JsonResponse: List of enrollment records or error message.
    """
    try:
        if request.method == 'GET':
            ...
            users = EnrolledUser.objects.filter(created_at__range=(start_datetime, end_datetime))
